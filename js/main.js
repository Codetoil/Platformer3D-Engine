...
class Main {
  ...
  static moveCh(ch, cameraAlpha, x, y, onGround, onWall, isSprinting) {
    var r = Main.calcCh(x, y);
    if (r > 0.01) {
      let r1 = Math.abs(x) + Math.abs(y);
      x *= r / r1;
      y *= r / r1;
      var dir = new BABYLON.Vector3(-x * Math.cos(cameraAlpha) - y * Math.sin(cameraAlpha), 0, y * Math.cos(cameraAlpha) - x * Math.sin(cameraAlpha));
      if (onGround) {
        ch.setDirection(dir);
      }
      var vel = ch.physicsImpostor.getLinearVelocity();
      ch.physicsImpostor.setLinearVelocity(vel.add(dir.scale(onGround ? 1.5 : 0.8)));
    }
  }
  static jumpCh(ch) {
    var vel = ch.physicsImpostor.getLinearVelocity();
  ...
  static CreateScene(engine, canvas) {
    ...
    // Lights
    var _lightHemi = new BABYLON.HemisphericLight("hemi", new BABYLON.Vector3(0, 1, 0), scene);
    //Simple crate
    var player = BABYLON.MeshBuilder.CreateCapsule("player", {
      radius: 0.75,
      height: 3,
      subdivisions: null,
      tessellation: null,
      capSubdivisions: null
    }, scene);
    ...
    player.physicsImpostor = new BABYLON.PhysicsImpostor(player, BABYLON.PhysicsImpostor.CapsuleImpostor, {
      mass: 1
    }, scene);
    var onGround = true;
    var camera = new BABYLON.ArcFollowCamera("camera", Math.PI / 2, 0.5, 10, player, scene);
    ...
    walls.push(wall2);
    var platform = BABYLON.MeshBuilder.CreateBox("platform1", {
      width: 5.0,
      depth: 5.0,
      height: 0.5
    }, scene);
    ...
    var dbox = BABYLON.MeshBuilder.CreateBox("dbox", {
      width: 1,
      height: 2,
      depth: 1
    }, scene);
    ...
    scene.onBeforeRenderObservable.add(function (evt) {
      ...
      var onGround = grounds.map(gnd => player.intersectsMesh(gnd, false) ? player.intersectsMesh(gnd, true) : false).reduce((p, c) => p || c, false);
      var onWall = walls.map(wll => [wll, player.intersectsMesh(wll, false) ? player.intersectsMesh(wll, true) : false]);
      var maxHSpeed = 10.0;
      var isSprinting = false;
      var isJumping = false;
      var moveArray = [];
      if (deviceSourceManager.getDeviceSource(BABYLON.DeviceType.Keyboard)) {
        let keyboardSource = deviceSourceManager.getDeviceSource(BABYLON.DeviceType.Keyboard);
        isSprinting = isSprinting || keyboardSource.getInput(16) === 0 || keyboardSource.getInput(76) === 0;
        moveArray.push([keyboardSource.getInput(87) - keyboardSource.getInput(83), keyboardSource.getInput(68) - keyboardSource.getInput(65), player, onGround, onWall, isSprinting]);
        moveArray.push([keyboardSource.getInput(38) - keyboardSource.getInput(40), keyboardSource.getInput(39) - keyboardSource.getInput(37), player, onGround, onWall, isSprinting]);
        isJumping = isJumping || keyboardSource.getInput(32) === 1 || keyboardSource.getInput(74) === 1;
      }
      if (deviceSourceManager.getDeviceSource(BABYLON.DeviceType.Generic)) {
        let gamepadSource = deviceSourceManager.getDeviceSource(BABYLON.DeviceType.Generic);
        isSprinting = isSprinting || gamepadSource.getInput(0) === 1 || gamepadSource.getInput(3) === 1;
        moveArray.push([-gamepadSource.getInput(15), gamepadSource.getInput(14), player, onGround, onWall, isSprinting]);
        isJumping = isJumping || gamepadSource.getInput(1) === 1 || gamepadSource.getInput(2) === 1;
      }
      if (deviceSourceManager.getDeviceSource(BABYLON.DeviceType.Switch)) {
        let gamepadSource = deviceSourceManager.getDeviceSource(BABYLON.DeviceType.Switch);
        isSprinting = isSprinting || gamepadSource.getInput(3) === 1 || gamepadSource.getInput(2) === 1;
        moveArray.push([-gamepadSource.getInput(23), gamepadSource.getInput(22), player, onGround, onWall, isSprinting]);
        isJumping = isJumping || gamepadSource.getInput(0) === 1 || gamepadSource.getInput(1) === 1;
      }
      if (deviceSourceManager.getDeviceSource(BABYLON.DeviceType.DualShock)) {
        let gamepadSource = deviceSourceManager.getDeviceSource(BABYLON.DeviceType.DualShock);
        isSprinting = isSprinting || gamepadSource.getInput(3) === 1 || gamepadSource.getInput(2) === 1;
        moveArray.push([-gamepadSource.getInput(19), gamepadSource.getInput(18), player, onGround, onWall, isSprinting]);
        isJumping = isJumping || gamepadSource.getInput(0) === 1 || gamepadSource.getInput(1) === 1;
      }
      //console.log(moveArray);
      let moveRslt = moveArray.reduce((p, c, i) => {
        var r = Main.calcCh(c[0], c[1]);
        return p[0] < r ? [r, i] : p;
      }, [0, null]);
      maxHSpeed *= moveRslt[0];
      if (moveRslt[1] != null) {
        Main.moveCh(moveArray[moveRslt[1]][2], camera.alpha, moveArray[moveRslt[1]][0], moveArray[moveRslt[1]][1], moveArray[moveRslt[1]][3], moveArray[moveRslt[1]][4], moveArray[moveRslt[1]][5]);
      }
      if (!isJumping) {
        hasJumped = false;
      }
      if (isJumping) {
        ...
        if (!onGround && !onWall.reduce((p, c) => p || c[1], false)) {
          ...
          player.physicsImpostor.setLinearVelocity(vel.add(new BABYLON.Vector3(0, 0.5, 0)));
        }
      } else {
        canWallJump = true;
      }
      ...
      player.physicsImpostor.setLinearVelocity(new BABYLON.Vector3(velxz.x, vely, velxz.z));
    ...
    for (var i = 1; i < scene.meshes.length; i++) {
      if (scene.meshes[i].checkCollisions && scene.meshes[i].isVisible === false) {
        scene.meshes[i].physicsImpostor = new BABYLON.PhysicsImpostor(scene.meshes[i], BABYLON.PhysicsImpostor.BoxImpostor, {
          mass: 0,
          friction: 0.5,
          restitution: 0.7
        }, scene);
